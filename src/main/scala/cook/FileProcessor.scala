...
class FileProcessor(ui: UI) {
  ...
  def IOWrite() = {
    ...
    for ((food, num) <- menu.foodList) {
      ...
      pw.write(
        food.name + "\t" + ingredientsString + "\t" + food.main_unit + "\t" + food.second_unit + "\t" + food.density.toString + "\t" + food.tag + "\t" + food.description + "\t" + food.isMenu + "\t" + num.toString + "\n"
      )
    }
    pw.close()
  }
  ...
  def lineProcessor(line: String, container: ArrayBuffer[Array[Any]]): Unit = {
    ...
    val isMenuAdd: Boolean =
      if (splitted(7) == "1") true
      else if (splitted(7) == "0") false
  ...
  def loadFromIO() = {
    ...
    try {
        ...
        try {
          for (data <- buffer) {
            ...
            if (menu.allIngredientsExist(ingredientNames)) {
              val ingreMapped =
                ingredients.map(x => (menu.returnFoodWithName(x._1).get, x._2))
      ...
      ui.p(
        "Notice: " + menu.menuFoodlist.size.toString + " menus and " + menu.nonMenuFoodlist.size.toString + " ingredients have been imported"
      )
    } catch {
      case _: IOException =>
        ui.leftFeedback.text =
          "> Errors in file. Please check your input file. "
